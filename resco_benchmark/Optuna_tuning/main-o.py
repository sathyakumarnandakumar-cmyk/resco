...
and pruning. Logs every trial to Neptune. Currently supports IDQN only.
...
# ---------------------------------------------------------------------------
# Neptune logging (reused from main.py)
# ---------------------------------------------------------------------------
def log_metrics(buf_infos: dict, run: Run, done: bool, mode: str):
    """Log step/episode metrics to Neptune. Identical to main.py."""
    if not done:
        ...
        chosen_routes = [
            "Infout-HLin", "PBBN-FMin", "PBBN-SirimS", "PBBN-SirimW",
            "PBBN-SKE", "PBBW-FMin", "PBBW-SKE", "SirimE-HLin",
            "SirimS-HLin", "SirimS-PBBN", "SirimW-HLin", "SirimW-SirimE",
            "SKE-HLin", "SKE-PBBN",
        ]
        for route_id in buf_infos["routes"].keys():
            if route_id in chosen_routes:
                ...
                run[
                    "metrics/" + mode + "/routes/" + route_id + "/Delay_Idx_StDev"
                ].log(buf_infos["routes"][route_id]["Delay_Idx_StDev"])
                run[
                    "metrics/" + mode + "/routes/" + route_id
...
def objective(trial: optuna.Trial, args):
    ...
    reports validation metrics for pruning, and logs everything to Neptune.
    """
    ...
    if args.map == "BB5B":
        ...
        run = neptune.init_run(
            name=f"{args.agent}-optuna-trial{trial.number}-{args.map}",
            description=f"Optuna trial {trial.number}: {args.agent} on {args.map}",
            tags=[
                "sumo-v0",
    ...
    try:
        for i in range(1, args.eps_val * args.validation_interval + 1):
            ...
            if mode == "validation" and args.map == "BB5B":
                ...
                # Log both metrics to Neptune
                if run is not None:
                    run["optuna/avg_delay"].append(avg_delay)
                    run["optuna/vehicles_completed"].append(vehicles_completed)
                    run["optuna/validation_step"].append(validation_step_counter)
        ...
        if dict_with_agents:
            ...
            if run is not None:
                ...
                run["optuna/status"] = "completed"
                run["optuna/final_avg_delay"] = final_avg_delay
                run["optuna/final_vehicles_completed"] = final_vehicles
        ...
        try:
            ...
            pass
        if run is not None:
            run["optuna/status"] = "failed"
            run["optuna/error"] = str(e)
            run.stop()
        raise
